...
def _generate_task(seed, d, n):
    ...
    return mvsn.sample(rng, (xi, Omega, eta), n)
...
def main():
    ...
    def train(key, state):
        def _step(s, k):
            return step(s, lambda _: -mvsn.logprob(_, get_batch(k)).mean(0), )
        return scan(_step, state, split(key, args.nsteps_per_eval))
    
    def eval(state):
        return -mvsn.logprob(params(state), x_test).mean(0)

...
def _sngd_step(args, state, optfuns, f):
    ...
    loss, g = jax.value_and_grad(_f, [0,1,2])(mvn.meanparams(natparams), *opt_params(opt_state))
...
def _elbo(rng, n, x, y, regularisation, qparams, nsamples):
    Ez = mvst.mean(qparams)
    Vz = mvst.var(qparams)
    Ezz = diagv(Vz) + Ez*Ez
    Elogp = jnp.sum(-.5*Ezz*regularisation - .5*jnp.log(2*jnp.pi/regularisation), 0)
    z = mvst.sample(rng, qparams, nsamples)
    Elogq = mvst.logprob(qparams, z).mean(0)
